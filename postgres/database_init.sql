...
create table if not exists load_layer.sight_images (
	id serial not null,
	sight_name VARCHAR(100) not null,
...
create table if not exists integration_layer.dim_sights_sights(
...
create table if not exists integration_layer.fact_sights(
	...
	primary key (city_id, timestamp_id, image_id, resolution_id, sight_id)
);
...
CREATE MATERIALIZED VIEW if not exists data_mart_layer.trained_models as (
	select cities.city_name, models.output_sight_mapping, models.trained_model_model
...
begin
	...
	if not exists (SELECT 1 FROM pg_constraint WHERE conname = 'sights_sight_id_fk') THEN
		alter table integration_layer.fact_sights 
		add constraint sights_sight_id_fk 
		foreign key (sight_id) 
		references integration_layer.dim_sights_sights (sight_id);
	end if;
...
CREATE OR REPLACE FUNCTION load_images_into_dwh()
  ...
  declare formatted_city VARCHAR(100) := NULL;
	...
	declare temp_sight_key INTEGER := NULL;   
...
begin	
	...
	-- get sights images dimension table key
	temp_sight_key := (select sight_id from integration_layer.dim_sights_sights where sight_name = new.sight_name limit 1);
	if temp_sight_key is null then
		INSERT INTO integration_layer.dim_sights_sights(sight_name) values (new.sight_name);
		temp_sight_key := (select sight_id from integration_layer.dim_sights_sights where sight_name = new.sight_name limit 1);
	end if;
	...
	temp_fact_key := (select count(*) from integration_layer.fact_sights as facts
						where facts.sight_id = temp_sight_key and facts.city_id = temp_city_key and facts.image_id = temp_image_key and 
							facts.resolution_id = temp_resolution_key and facts.timestamp_id = temp_timestamp_key);
	if temp_fact_key = 0 then
		insert into integration_layer.fact_sights(sight_id, city_id, timestamp_id, image_id, resolution_id) 
			values (temp_sight_key, temp_city_key, temp_timestamp_key, temp_image_key, temp_resolution_key);
	end if;
	-- create 
	...
	temp_city_images_data_mart_query := format('select image_data.image_file as image_file, image_data.sight_name as sight_name, 
		image_data.resolution_width as resolution_width, image_data.resolution_height as resolution_height
		from (
			select distinct(images.image_source) as url, images.image_file as image_file, 
				resolutions.resolution_height as resolution_height, resolutions.resolution_width as resolution_width, sights.sight_name as sight_name
			from integration_layer.dim_sights_images as images, 
				...
				integration_layer.dim_sights_sights AS sights
			WHERE facts.city_id = %s and resolutions.resolution_id = facts.resolution_id and 
				images.image_id = facts.image_id and sights.sight_id = facts.sight_id) as image_data', temp_city_key);
