...
def trigger_city_image_labelling() -> None:
    ...
    """
    ils_training_service_url = environ['ILS_ENDPOINT_URL'][:-1] if environ['ILS_ENDPOINT_URL'][-1] == '/' \
        else environ['ILS_ENDPOINT_URL']
...
def trigger_city_model_training() -> None:
    ...
    POST request with the city name as a path parameter.
    """
    mts_training_service_url = environ['MTS_ENDPOINT_URL'][:-1] if environ['MTS_ENDPOINT_URL'][-1] == '/' \
        else environ['MTS_ENDPOINT_URL']
