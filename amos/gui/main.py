...
class UiMainWindow(QWidget):
	...
	def show_popup(self) -> None:
		...
		if city != "Choose City":
			...
			city_dos = city
			city_dos[city_dos.index(' ')] = '_'
			print(city_dos)
			latest_version = get_dwh_model_version(city_dos)
			...
			if downloaded_version == -1:
				...
				msg.setText("Do you want to download " + city + "?")
				msg.setIcon(QMessageBox.Question)
				msg.setStandardButtons(QMessageBox.Cancel | QMessageBox.Ok)
				msg.setDefaultButton(QMessageBox.Ok)
				msg.setInformativeText("When downloaded sights of " + city + " can be detected.")
				...
				newest_vers_msg.setText("You can start detecting sights in " + city + "!")
	...
	def handover_city(self, button) -> None:
		...
		if button.text() == "OK":
			...
			city_dos = city
			city_dos[city_dos.index(' ')] = '_'
			model = get_downloaded_model(city_dos)
