...
from dwh_communication.exec_sql import exec_dml_query, exec_dql_query
from mock import patch
import pytest


def test_valid_dql_query(connection_mock):
    with patch("dwh_communication.exec_sql.connect", return_value=connection_mock) as conn_mock, patch(
        "dwh_communication.exec_sql.config", return_value={}
    ) as config_mock:
...
def test_invalid_query(connection_exception_mock):
    with patch("dwh_communication.exec_sql.connect", return_value=connection_exception_mock) as conn_mock, patch(
        "dwh_communication.exec_sql.config", return_value={}
    ) as config_mock:
        assert (conn_mock.called or config_mock.called) is False
