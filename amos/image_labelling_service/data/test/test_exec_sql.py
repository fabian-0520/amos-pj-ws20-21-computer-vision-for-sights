...
def test_valid_dql_query(connection_mock):
    with patch("data.exec_sql.connect", return_value=connection_mock) as conn_mock, patch(
        "data.exec_sql.config", return_value={}
    ) as config_mock:
...
def test_invalid_query(connection_exception_mock):
    with patch("data.exec_sql.connect", return_value=connection_exception_mock) as conn_mock, patch(
        "data.exec_sql.config", return_value={}
    ) as config_mock:
        assert (conn_mock.called or config_mock.called) is False
