...
from data.handler import upload_image, upload_image_labels, _get_image_label_dml_query, get_downloaded_model
from mock import patch, PropertyMock


def test_upload_image(in_memory_file_mock, md5_mock):
    with patch('data.handler.Image', new_callable=PropertyMock, return_value=in_memory_file_mock), \
            patch('data.handler.md5', return_value=md5_mock), \
            patch('data.handler.exec_dml_query'):
        assert upload_image(in_memory_file_mock, 'Berlin') == md5_mock.hexdigest()


def test_upload_image_labels(labels_mock, md5_mock):
    with patch('data.handler.exec_dml_query') as exec_mock:
...
def test_get_downloaded_model_found(model_mock):
    with patch('data.handler.exec_dql_query', return_value=[[model_mock]]):
        result_model = get_downloaded_model('berlin')
        assert result_model == model_mock.tobytes()


def test_get_downloaded_model_not_found():
    with patch('data.handler.exec_dql_query', return_value=[]) as mocked_models:
        result_model = get_downloaded_model('berlin')
